...
RSpec.describe "expect { ... }.to raise_error" do
  ...
  it "can supresses the warning when configured to do so", :potential_false_positives do
...
RSpec.describe "expect { ... }.not_to raise_error('message')" do
  ...
  it "supresses the warning when configured to do so", :potential_false_positives do
    RSpec::Expectations.configuration.on_potential_false_positives = :nothing
    expect_no_warnings
    expect { raise 'blarg' }.not_to raise_error('blah')
  end
