    ...
    module BuiltIn
      module Helpers
        ...
        class ArrayMatcher
          ...
          def initialize(expected:, actual:, messages:, display_array: ->(array) { array })
            @_expected = expected
            @_actual = actual
          ...
          def expected_collection_line
            message_line(@messages[:expected], expected, true)
          end

          def actual_collection_line
            message_line(@messages[:actual], actual)
          end

          def missing_elements_line
            message_line(@messages[:missing], missing_items, true)
          end

          def extra_elements_line
            message_line(@messages[:extra], extra_items)
          end

          def describe_collection(collection, surface_descriptions=false)
            if surface_descriptions
              "#{description_of(@display_array.call(surface_descriptions_in collection))}\n"
            else
              "#{description_of(@display_array.call(collection))}\n"
            end
          end
