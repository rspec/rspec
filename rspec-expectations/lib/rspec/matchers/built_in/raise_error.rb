      ...
      private
        ...
        def raise_nil_argument_error!
          raise ArgumentError, "Using the `raise_error` matcher with a `nil` " \
                               "value is probably unintentional, it risks false positives, " \
                               "since `raise_error` will match when Ruby raises a " \
                               "`NoMethodError`, `NameError` or `ArgumentError`, potentially " \
                               "allowing the expectation to pass without even executing the " \
