Feature: Thread dump on SIGINFO / SIGPWR
  ...
  Scenario: Sending the backtrace signal during a run prints a thread dump to stderr
    Given a file named "spec/slow_spec.rb" with:
      """ruby
      ...
      """
    When I run `rspec spec/slow_spec.rb` with the backtrace signal sent during the run
    Then the output should contain "spec/slow_spec.rb"
    And the output should contain %R{Thread TID-[a-z0-9]+ <[^>]*> .*/spec/slow_spec\.rb:3:in (?:`sleep'|'Kernel#sleep')}
